	...
	public static function singleHandle($that,$select) {
		if (preg_match_all("/<h(\d)>(.*)<\/h\d>/isU",$that->content,$outarr)) {
			...
			for ($key=0; $key<count($outarr[2]);$key++) {
				...
				$that->content = substr($ta, 0, $tb)."<a name=\"{$key}\"></a>".substr($ta, $tb);
				...
				array_push($index,
					array(
						"level"=>$outarr[1][$key],
						"link"=>"<a class='jump' href=\"#{$key}\">{$outarr[2][$key]}</a>"
					)
				);
			}
			$curlevel = 0;
...
// 			$that->widget = "<div id=\"theContentIndex\">{$index_out}</div>". $that->widget;
		}
	}
}
